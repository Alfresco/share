...
public abstract class BaseTest extends TestCase {
	...
	public BaseTest() {
		...
		// Try to get resources dir from a system property otherwise uses the default
		// hardcoded
		// backward compatible
		String resourcesDir = null;
		// This allows subclasses to override the getResourcesDir method (e.g. by
		// passing classpath: )
		if (getResourcesDir() == null) {
			...
			if (resourcesDir == null || resourcesDir.equals("")) {
				...
				resourcesDir = System.getProperty("user.dir") + File.separator + "source" + File.separator
						+ "test-resources";
			}
		} else {
		...
		// Returns the resources dir with trailing separator or just the classpath:
		// string in case that was specified
		this.resourcesDir = resourcesDir + ((loadFromClasspath) ? "" : File.separator);
	}

	/**
	 * Override this method to pass a custom resource dir. Valid values are a file
	 * system path or the string "classpath:"
	 * 
	 * @return
	 */
	 ...
	 * 
	 * In case of file resource, provide the full file path In case of classpath
	 * resources, please pass the full resource URI, prepended with the classpath:
	 * string
	 * 
	 * @param fullFileName
	 */
	 ...
	 * In case of classpath resources, just provide the resource URI, without with
	 * the prepending classpath: string
	 * 
	 * @param xmlConfigFile
	 * @return
	 */
	...
	protected XMLConfigService initXMLConfigService(List<String> xmlConfigFilenames) {
		...
		for (String filename : xmlConfigFilenames) {
			// if we load from classpath then no need to prepend the resources dir (which
			// will be .equals("classpath:")
			String path = ((loadFromClasspath) ? "" : getResourcesDir()) + filename;
			assertFileIsValid(path);
			configFiles.add(path);
		}
		ConfigSource configSource = (loadFromClasspath) ? new ClassPathConfigSource(configFiles)
	...
	protected XMLConfigService initXMLConfigServiceWithProperties(List<String> xmlConfigFilenames,
			Resource[] resources) {

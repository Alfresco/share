 ...
 * Changes:
 * 
 * Multi-line changes are marked with // Alfresco Changes Start , // Alfresco Changes End
 *
 * line 677  - Added the option of expanding and collapsing also the nodes with custom style.
 * line 1561 - Added attribute customCls for YAHOO.widget.Node. This attribute is used to set different style for Tree Node if it is not null.
 * line 2129 - Changed updateIcon function in order to change current style when expanding and collapsing also for nodes that have custom style. 
 * line 2171 - If customCls attribute is not null, this will be the tree node style, otherwise the default style will be used.
 * line 2883 - Added _customRegEx function. This result is used to decide if nodes that have custom style will be expanded or collapsed.
 */
...
/**
  ...
  /**
    ...
    _onClickEvent: function (ev) {
        ...
        // Alfresco Changes Start
        else if (node._customRegEx(node.customCls) && node._customRegEx(node.customCls).test(td.className))
        {
           toggle(true);
        }
        // Alfresco Changes End
        else {
...
YAHOO.widget.Node.prototype = {
     ...
     */
     hasIcon: true,
    /**
     * Attribute used for setting custom style class for tree node.
     */
     customCls: null,
    /**
...
*/
  ...
  /**
    ...
    updateIcon: function() {
        if (this.hasIcon) {
            ...
            if (el) {
               // Alfresco Changes Start
               if (this._customRegEx(this.customCls) && this._customRegEx(this.customCls).test(el.className))
               {
                  el.className = el.className.replace(this._customRegEx(this.customCls), this.getStyle());
               }
               else
               {
  ...
  /**
    ...
    */
    ...
    // Alfresco Changes Start
    /**
     * Gets a RegExp based on custom style. This is used for expanding collapsing nodes that have custom style. 
     * @param customCls
     * @returns {RegExp}
     */
   _customRegEx : function(customCls)
   {
      var customRegEx = null;
      if (customCls != null)
      {
         customRegEx = new RegExp(customCls+"[mpn]", "g");
      }
      return customRegEx;
   }
   // Alfresco Changes End

...
/**
 * Dashboard MyMeetingWorkspaces component.
 * 
 * @namespace Alfresco.dashlet
 * @class Alfresco.dashlet.MyMeetingWorkspaces
 */
(function()
{
   /**
   ...
   /**
    * Dashboard MyMeetingWorkspaces constructor.
    * 
    * @param {String} htmlId The HTML id of the parent element
    * @return {Alfresco.dashlet.MyMeetingWorkspaces} The new component instance
    * @constructor
    */
   Alfresco.dashlet.MyMeetingWorkspaces = function MyMeetingWorkspaces_constructor(htmlId)
   {
      Alfresco.dashlet.MyMeetingWorkspaces.superclass.constructor.call(this, "Alfresco.dashlet.MyMeetingWorkspaces", htmlId, ["button", "container", "datasource", "datatable", "animation"]);
   ...
   YAHOO.extend(Alfresco.dashlet.MyMeetingWorkspaces, Alfresco.component.Base,
   {
      /**
       ...
       * @method onReady
       */
      onReady: function MyMeetingWorkspaces_onReady()
      {
         var me = this;
         ...
         var renderCellFavourite = function MS_oR_renderCellFavourite(elCell, oRecord, oColumn, oData)
         {
            ...
            desc += '<a class="favourite-site ' + favEventClass + (isFavourite ? ' enabled' : '') + '" title="' + me.msg("link.favouriteMeetingWorkspace") + '">&nbsp;</a>';
            if (me.options.imapEnabled)
            {
               desc += '<a class="imap-favourite-site ' + imapEventClass + (isIMAPFavourite ? ' imap-enabled' : '') + '" title="' + me.msg("link.imap_favouriteMeetingWorkspace") + '">&nbsp;</a>';
            }
            desc += '</div>';
         ...
         var renderCellActions = function MS_oR_renderCellActions(elCell, oRecord, oColumn, oData)
         {
            ...
            if (isSiteManager)
            {
               desc = '<a class="delete-site ' + deleteEventClass + '" title="' + me.msg("link.deleteMeetingWorkspace") + '">&nbsp;</a>';
            }
            elCell.innerHTML = desc;
         };
       ...
       */
      onDeleteSite: function MyMeetingWorkspaces_onDeleteSite(row)
      {
       ...
       */
      onSiteDeleted: function MyMeetingWorkspaces_onSiteDeleted(layer, args)
      {
       ...
       * @param row {object} DataTable row representing file to be actioned
       */
      onFavouriteSite: function MyMeetingWorkspaces_onFavouriteSite(row)
      {
         var record = this.widgets.dataTable.getRecord(row),
            site = record.getData(),
            siteId = site.shortName;
         ...
         var responseConfig =
         {
            failureCallback:
            {
               fn: function MS_oFS_failure(event, obj)
               {
                  ...
                  {
                     text: this.msg("message.meetingWorkspaceFavourite.failure", site.title)
                  });
               },
               scope: this,
               obj:
       ...
       * @param row {object} DataTable row representing file to be actioned
       */
      onImapFavouriteSite: function MyMeetingWorkspaces_onImapFavouriteSite(row)
      {
         var record = this.widgets.dataTable.getRecord(row),
            site = record.getData(),
            siteId = site.shortName;
       ...
       */
      onCreateSiteLinkClick: function MyMeetingWorkspaces_onCreateSiteLinkClick(event)
      {
       ...
       */
      _findRecordByParameter: function MyMeetingWorkspaces__findRecordByParameter(p_value, p_parameter)
      {

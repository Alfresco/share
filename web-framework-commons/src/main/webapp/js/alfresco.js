...
/**
 * Given a fileName, if the provided mimetype is empty then it returns the mimetype that coresponds to file name extension.
 * @param fileName {String} - provided file name
...
Alfresco.util.getMimetypeByFileNameExtention = function(fileName, type)
{
   ...
   var newVersionMimetype = type;
   if(newVersionMimetype === "")
   {
      var fileExtension = "generic"
      var i = fileName.lastIndexOf(".");
      if (i > -1)
      {
         fileExtension = fileName.substring(i + 1).toLowerCase();
      }
      if(fileExtension in extns)
      {
         newVersionMimetype = extns[fileExtension];
      }
   }
   return newVersionMimetype;
};
